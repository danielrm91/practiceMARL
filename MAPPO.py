...
#
# 2. Policy and Centralized Critic
#
class Actor(nn.Module):
    ...
    def get_action_and_logprobs(self, obs:torch.Tensor, actions:torch.Tensor) -> Tuple[torch.Tensor,torch.Tensor]:
...
class RolloutBbuffer:
    ...
    def to_tensor(self, device:str) -> Dict[str, torch.Tensor]:
        return{
            "obs":torch.tensor(np.array(self.obs),dtype=torch.float32,dvice=device),
            "global_state":torch.tensor(np.array(self.global_state),dtype=torch.float32,device=device),
            "actions":torch.tensor(np.array(self.action),dtype=torch.long,device=device),
...
class MAPPO:
    ...
    def select_actions(self,obs:np.ndarray) -> Tuple[np.ndarray,np.ndarray,float]:
        ...
        with torch.no_grad():
            for i, actor in enumerate(self.actors):
                action_i,logprob_i = actor.get_action_and_logprobs(obs_t[i].unsqueeze(0))
    ...
    def update(self, buffer:RolloutBbuffer, next_obs:np.ndarray):
        cfg = self.cfg
        ...
        flat_obs = obs.reshape(T * cfg.n_agents, cfg.obs_dime)
        ...
        for _ in range(cfg.ppo_epochs):
            ...
            for start in range(0, batch_size, cfg.minibatch_size):
                ...
                # Identify which actor each sample belongs to (wow)
                ...
                nn.utils.clip_grad_norm(self.actors.parameters(),cfg.max_grad_norm)
            ...
            nn.utils.clip_grad_norm(self.critic.parameters(),cfg.max_grad_norm)
...
class DummyMultiAgentEnv:
    ...
    def reset(self) -> np.ndarray:
        self.step_count = 0
        return np.random.random(self.n_agents, self.obs_dim).astype(np.float)
